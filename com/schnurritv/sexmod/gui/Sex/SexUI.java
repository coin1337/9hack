   ...
   public void renderUI(RenderGameOverlayEvent event) {
      if (shouldBeRendered && event.getType() == ElementType.TEXT) {
         ...
         if (!keepSpacePressed) {
            ...
            this.func_73729_b((int)PenisMath.Lerp(-200.0D, 98.0D, (double)transitionStep), 405, 0, height, 158, 54);
         }

         GL11.glScalef(2.857143F, 2.857143F, 2.857143F);
         minecraft.field_71446_o.func_110577_a(hornyMeter);
         GL11.glScalef(0.75F, 0.75F, 0.75F);
         this.func_73729_b(10, (int)PenisMath.Lerp(-200.0D, 10.0D, (double)transitionStep), 0, 0, 146, 175);
         drawnCumPercentage = PenisMath.Lerp(drawnCumPercentage, cumPercentage, (double)minecraft.func_193989_ak());
         height = (int)PenisMath.Lerp(0.0D, 160.0D, drawnCumPercentage);
         int textureY = (int)PenisMath.Lerp(167.0D, 8.0D, drawnCumPercentage);
         double y = PenisMath.Lerp(178.0D, 18.0D, drawnCumPercentage);
         if (!keepSpacePressed) {
            this.func_73729_b(67, (int)PenisMath.Lerp(-45.0D, y, (double)transitionStep), 159, textureY, 32, height);
            this.func_73729_b(120, (int)PenisMath.Lerp(-58.0D, PenisMath.Lerp(178.0D, 149.0D, 1.0D - drawnCumPercentage), (double)transitionStep), 212, (int)PenisMath.Lerp(169.0D, 141.0D, 1.0D - drawnCumPercentage), 28, (int)PenisMath.Lerp(1.0D, 29.0D, 1.0D - drawnCumPercentage));
            this.func_73729_b(18, (int)PenisMath.Lerp(-58.0D, PenisMath.Lerp(178.0D, 149.0D, 1.0D - drawnCumPercentage), (double)transitionStep), 212, (int)PenisMath.Lerp(169.0D, 141.0D, 1.0D - drawnCumPercentage), 28, (int)PenisMath.Lerp(1.0D, 29.0D, 1.0D - drawnCumPercentage));
         } else {
            cumStep += minecraft.func_193989_ak() / 15.0F;
            this.func_73729_b(67, (int)PenisMath.Lerp(18.0D, -300.0D, (double)cumStep), 159, 8, 32, 160);
         }
